...
@Component({
  selector: 'app-croffle',
  standalone: false,
  templateUrl: './croffle.component.html',
  styleUrl: './croffle.component.css'
})
export class CroffleComponent implements OnInit{
products: Product[] = [];
  categories: Category[] = [];
constructor(private productService: ProductService, private categoryService: CategoryService,private cartService: CartService, private http: HttpClient) { }
  ...
  ngOnInit(): void {
    ...
    this.categoryService.getProductsByCategory('CROF').subscribe(data => {
